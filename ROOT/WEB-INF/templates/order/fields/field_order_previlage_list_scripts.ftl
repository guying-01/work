...
<script type="text/javascript">
    ...
    var pageNum = 1;         //加载更多标志页数
    var totalDataArr = []    //加载更多用来存储拼接数据
    ...
    function HbPayDefault() {
        ...
        var str = "";
        str = str + '<form action=' + payUrl + ' id="toPayForm" name="toPayForm" method="post"> <input type="hidden" id="showHeadBranchInfo" name="showHeadBranchInfo" value="0"/><input type="hidden" id="orderIds" name="orderIds" value="" /><input type="hidden" id="money" name="money" value="" />'
...
//		+'" class="small-button">合并付款</a></form>';
    ...
    function checkHbPay() {
        ...
        str = str + '<form action=' + payUrl + ' id="toPayForm" name="toPayForm" method="post"><input type="hidden" id="showHeadBranchInfo" name="showHeadBranchInfo" value="0"/><input type="hidden" id="orderIds" name="orderIds" value="" />'
//        +'<input type="checkbox" id="parentCheckbox" onclick="allSeclect(this)"/>'
//		+ '<span>全选</span><a style="cursor: pointer;" onclick="megerPayment('
//		+"'',0)"
//		+'" class="small-button">合并付款</a></form>';
    ...
    function searchOrder(page) {
        ...
        callService("${ctx}/order/findOrderList.ajax",
                ...
                },
                {
                    ...
                    var recordsTotal = datas.paginationMapResponse.recordsTotal;
                    var maxPageNum = Math.ceil(recordsTotal / 10);
                    totalDataArr = totalDataArr.concat(data);
                    if (page < maxPageNum) {
                        $(".orderMore").text("加载更多").removeClass("nomore").attr("onclick", "orderMore()");
                    }
                    else {
                        $(".orderMore").text("没有更多啦~").addClass("nomore").attr("onclick", "");
                    }
                    ...
                    if (datas != "") {
                        ...
                        <#--if(checkResult != null){-->
                        <#--if(checkResult == 0){-->
                        <#--htmlStr += '<br/><a href="javascript:applyRefund(\''+order.id+'\',\''+order.orderDetailVoList[detailIndex].id+'\',0)" class=" font-color-c">申请退款</a>';-->
                        <#--}else{-->
                        <#--htmlStr += '<br/><a href="javascript:applyRefund(\''+order.id+'\',\''+order.orderDetailVoList[detailIndex].id+'\',1)" class=" font-color-c">修改退款</a>';-->
                        <#--}-->
                        <#--}-->
                        <#--}-->
                        <#--// 查看退款/退货详情按钮-->
...
//								htmlStr += '<a class="font-color-c" href="javascript:toOrderDetail(\''+ order.id +'\')">查看订单详情</a>';
                                return new Handlebars.SafeString(htmlStr);
                            });
                            // '操作'字段的处理
...
//										htmlStr = '<span class="table_bgA" >待验收</span>';
                            ...
                            /*'支付时间'字段的处理*/
                            Handlebars.registerHelper('show_tradeAsynNotifyTime', function (tradeAsynNotifyTime, opt) {
                                if (tradeAsynNotifyTime) {
                                    return ' <p class=" m-r-10"> 支付时间：' + tradeAsynNotifyTime + '</p>';
                                } else {
                                    return '';
                                }
                            });
                            /*'支付状态'字段的处理*/
                            Handlebars.registerHelper('show_payStatus', function (payStatus, opt) {
                                if (payStatus) {
                                    return '<p class=" m-r-10"> 支付状态：' + payStatus + '</p>';
                                } else {
                                    return '';
                                }
                            });
                                ...
                                for (var i = 0; i < count(order_data); i++) {
                                    if (order_data[i].totalOrderId == group_id) {
                                        ...
                                        if ($("#showHeadBranchInfo").val() === "0") {
                                            ...
                                            //总子账号提示信息
                                            if (PREVILAGE || (orderMemberType && (orderMemberType === MEMBER_MASTER || orderMemberType === MEMBER_SEPARATE))) {
                                                var odMemberType = '';
                                                ...
                                                pay_bt += '<p class="od_storeName od_info">采购商店名：' + order_data[i].odBuyerName + '</p><p class="od_passportNo od_info">采购商身份码：' + order_data[i].odPassportNo + '</p><p class="od_memberType od_info">采购商账号类别：' + odMemberType + '</p>';
                                            }
                                        } else if ($("#showHeadBranchInfo").val() === "1") {
                                            //总子账号提示信息
                                            if (orderMemberType && (orderMemberType === MEMBER_HEAD || orderMemberType === MEMBER_MASTER || orderMemberType === MEMBER_SEPARATE)) {
                                                var odMemberType = '';
                                                if (order_data[i].odMemberType === MEMBER_HEAD) {
                                                    odMemberType = "分采集付主账号";
                                                } else if (order_data[i].odMemberType === MEMBER_BRANCH) {
