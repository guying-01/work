...
/**=========================更新登录密码====================================**/
	 $(document).ready(function(){
	    ...
	    $("#passwordsForm").validate({
			 ...
			 messages:{
				 passwordsShow:{
					 required:"<div class='label-g'>请输入原始密码</div>"
				 },
				 twoPasswordsShow:{
		...
		jQuery.validator.addMethod("checkNewPwd", function() {   
			...
			return check1}, "<div class='label-g'>原始密码与新密码相同，请重新输入</div>");
	...
	function validCurrentPwd(){
		...
		{callSuccess:function(status){
			if(status == "-1"){
				$("#currPwdWrong").css("display", "inline-block");
			}else{
	...
	function pwdStrongCheck(pwd){
		...
		switch(S_level) { 
			...
			case 3: 
				...
				$("#pwdStren2").css("display","none");
				$("#pwdStren3").css("display","none");
				$("#pwdStren4").css("display","inline-block");
				break;
			} 
	}
	...
	function protect1(){//答案1
		if($('[name="ppAnswer0"]').val()==''){
			$('#ppAnswer0Tip .label-g').show();return false;
		}else{
			$('#ppAnswer0Tip .label-g').hide();return true;
		}
	}
	function protect2(){//答案2
		if($('[name="ppAnswer1"]').val()==''){
			$('#ppAnswer1Tip .label-g').show();return false;
		}else{
			$('#ppAnswer1Tip .label-g').hide();return true;
		}
	}
	function protect3(){//答案3
		if($('[name="ppAnswer2"]').val()==''){
			$('#ppAnswer2Tip .label-g').show();return false;
		}else{
			$('#ppAnswer2Tip .label-g').hide();return true;
		}
	}
	function minlength0Check(){
		if($('[name="ppAnswer0"]').val()!=''){
			if($('[name="ppAnswer0"]').val().length<1){
			$('#minlength0Check .label-g').show();return false;
			}else{
				$('#minlength0Check .label-g').hide();return true;
			}
	...
	function minlength1Check(){
		if($('[name="ppAnswer1"]').val()!=''){
			...
			$('#minlength1Check .label-g').show();return false;
			}else{
				$('#minlength1Check .label-g').hide();return true;
			}
	...
	function minlength2Check(){
		if($('[name="ppAnswer2"]').val()!=''){
			...
			$('#minlength2Check .label-g').show();return false;
			}else{
				$('#minlength2Check .label-g').hide();return true;
			}
	...
	$(document).ready(function(){
		...
		$("#isMailForm").validate({
			 ...
			 messages:{
				 mailAddress:{
					 required:"<div class='label-g'>请填写邮箱地址</div>",
					 email:"<div class='label-g'>请输入正确的邮箱地址</div>"
				 }
			 }
		})
	});
