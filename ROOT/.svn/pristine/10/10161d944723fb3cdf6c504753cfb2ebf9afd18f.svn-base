...
<style>
    ...
    .only_book_content_zizhi .swiper-container {
        width: 4.38rem;
        height:7.8rem;
        overflow: hidden;
        position: relative;
    ...
    .only_book_content_zizhi .swiper-slide {
    	width: 4.38rem;
        float: left;
        text-align: center;
        height: 100%;
...
$(document).ready(function() {
    ...
    // 轮播配置
    function slider() {
        var wrap = $('.only_book_content_zizhi');
        // container容器
        var container = $('.only_book_content_zizhi .swiper-wrapper');
        // list列表
        var list = $('.only_book_content_zizhi .swiper-slide');
        // 左右箭头
        var prev = $('.only_book_content_zizhi .swiper-button-prev');
        var next = $('.only_book_content_zizhi .swiper-button-next');
        // 盒子宽度高度
    ...
    // 模态层配置
    function Active() {
        var motaicheng = $("#motaicheng1"); //模态层容器
        var lunbo = $(".only_book_content_zizhi .swiper-slide"); //获取轮播下的图片
        var lunbo_img = $(".only_book_content_zizhi .swiper-slide img");
        var lb_img_index = 0; //下标
        var mtc_img = $("#mtc_img1"); //模态层展示图
        ...
        lunbo.on("click", "img", function () {
        	...
        	$(".mtc_img_WH").css("top",($("body").height - $(".mtc_img_WH").height)/2 )
        ...
        function Mtc() {
            var mtc_button = $("#mtc_button1"); //模态层按钮
            var motaicheng = $("#motaicheng1"); //模态层容器
    ...
    $.zizhi = {}
    $.zizhi.getSliderImg = function(loadImg) {
        var str = '';
        ...
        $('.only_book_content_zizhi .swiper-wrapper').html(str);
        slider(); //加载轮播
        Active(); 
        //加载模态层
    }
...
<!--模态层开始-->
<div id="motaicheng1" class="motaicheng none">
    <div class="mtc_img_box">
        <div class="mtc_img_WH">
            <img id="mtc_img1" class="imgbgColor" src="" alt="">
        </div>        
    </div>
    <div id="mtc_button1" class="mtc_button">×</div>
    <div id="mtc_left1" class="mtc_btn mtc_left">&lt;</div>
    <div id="mtc_right1" class="mtc_btn mtc_right">&gt;</div>
</div>
<!--模态层结束-->
