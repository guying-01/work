...
<script type="text/javascript">
    ...
    function checkStatu() {
        ...
        if (localStorage.browsePage != '') {
            var page = JSON.parse(localStorage.browsePage);
            var id = JSON.parse(localStorage.index);
            $(".tabT li").removeClass('on').eq(id).addClass('on');
            pageNum = JSON.parse(localStorage.browsePage);
            getIndex(id);
            searchOrder(page, 'all');
        }
        else {
    ...
    function getIndex(id) {
        switch (id) {
            case 0:
                changeOrderState('', 0, '0');
                break;
            case 1:
                changeOrderState('00', 1, '1');
                break;
            case 2:
                changeOrderState('1020', 1, '2');
                break;
            case 3:
                changeOrderState('1000', 2, '3');
                break;
        }
    }
    ...
    function addToShopCart(totalOrderId, el) {
        ...
        callService(url, requestParams, {
            callSuccess: function (data) {
                ...
                for (var key in data) {
                    if (data[key].success) {
                        goods_mes += "<div>" + "商品 " + key + " 加入购物车成功" + "</div>";
                    }
                    else {
                        goods_mes += "<div>" + data[key].message + "</div>";
                    }
                }
                jsAlert(goods_mes);
                already_login();
    ...
    function HbPayDefault() {
        ...
        if (${getConstant('constant.PAYMENT_TYPE')}=="1"
    )
        {
            payUrl = "${ctx}/order/toPayFromOrderB2C.jhtml";
        }
        var str = "";
        str = str + '<form action=' + payUrl + ' id="toPayForm" name="toPayForm" method="post"> <input type="hidden" id="orderIds" name="orderIds" value="" /><input type="hidden" id="money" name="money" value="" />'
        //       +'<input type="checkbox" id="parentCheckbox" onclick="allSeclect(this)"/>'
//		+ '<span>全选</span><a style="cursor: pointer;" onclick="megerPayment('
//		+"'',0)"
//		+'" class="small-button">合并付款</a></form>';
        $("#allselect").html(str);
        if ($("#toPayForm").length > 0) {
            $("#toPayForm").append('<input type="hidden" id="showHeadBranchInfo" name="showHeadBranchInfo" value="0"/>');
        } else {
            $("body").append('<input type="hidden" id="showHeadBranchInfo" name="showHeadBranchInfo" value="0"/>');
        }
    }
    //检查是否有合并付款
    function checkHbPay() {
        ...
        str = str + '<form action=' + payUrl + ' id="toPayForm" name="toPayForm" method="post"><input type="hidden" id="orderIds" name="orderIds" value="" />'
//        +'<input type="checkbox" id="parentCheckbox" onclick="allSeclect(this)"/>'
//		+ '<span>全选</span><a style="cursor: pointer;" onclick="megerPayment('
//		+"'',0)"
//		+'" class="small-button">合并付款</a></form>';
        ...
        if (orderState == "") {
            $("#allselect").html(str);
        } else if (orderState == ${getConstant('dictionary.ORDER_STATES_INPAY')}) {
            $("#allselect").html(str);
        } else {
            $("#allselect").html("");
        }
    ...
    //订单状态切换
    function changeOrderState(orderState, type, showHeadBranchInfo) {
        var index = 0;
        switch (orderState) {
            case '':
                index = 0;
        ...
        $("#stateType").val(type);
        checkHbPay();
        if (showHeadBranchInfo) {
            $("#showHeadBranchInfo").val(showHeadBranchInfo);
        } else {
            $("#showHeadBranchInfo").val("");
        }
        searchOrder(1);
    }

    //按条件查询订单
    function searchOrder(page, all) {
...
//		$("#currentPage").val(page);
        ...
        $("tr").remove(".loading");
        if (all) {
            var browseLength = localStorage.browseLength;
            if (browseLength < 10 && browseLength != 0) {
                dataLength = browseLength + (page - 1) * 10;
                page = 1;
            }
            else if (browseLength == 0) {
                $("#noData").show();
                return
            }
            else if (browseLength == 10) {
                dataLength = 10 * page;
                page = 1;
            }
        }
        ...
        callService("${ctx}/order/findOrderList.ajax",
                {
                    ...
                    "length": dataLength,
                    "pageNumber": page,
                    "showHeadBranchInfo": $("#showHeadBranchInfo").val()
                },
                {
                    "async": false, callSuccess: function (datas) {
                    var data = datas.paginationMapResponse.data;
                    var maxNum = datas.paginationMapResponse.recordsTotal;
                    if (all) {
                        stateG = data;
                        localStorage.browseLength = JSON.stringify(data.length / pageG)
                    }
                    else {
                        stateG = stateG.concat(data);
                        localStorage.browseLength = JSON.stringify(data.length)
                    }
                    ...
                    if (datas != "") {
                        ...
                        /*转回数据格式结束*/
                        if (data.length > 0) {
                            var memberType = '${Session.user.memberType!''}';
                            if( $("#showHeadBranchInfo").val()==="1" && (MEMBER_HEAD == memberType || MEMBER_MASTER ==memberType)){
                                if($("#dt_box").attr("businessed")=="false"){
                                    $("#payAllEnable").hide();
                                    $("#payAllDisable").show();
                                }else{
                                    $("#payAllEnable").show();
                        ...
                        <#--if(checkResult != null){-->
                        <#--if(checkResult == 0){-->
                        <#--htmlStr += '<a href="javascript:applyRefund(\''+order.id+'\',\''+order.orderDetailVoList[detailIndex].id+'\',0)" class=" order_state_btn">申请退款</a>';-->
                        <#--}else{-->
                        <#--htmlStr += '<a href="javascript:applyRefund(\''+order.id+'\',\''+order.orderDetailVoList[detailIndex].id+'\',1)" class=" order_state_btn">修改退款</a>';-->
                        <#--}-->
                        <#--}-->
                        <#--}-->
                        <#--// 查看退款/退货详情按钮-->
                        ...
                        <#--if(returnType == ${getConstant('dictionary.RETURN_ORDER_TYPE_00')}){-->
                        <#--<!--退款状态&ndash;&gt;-->
                        <#--htmlStr += '<a href="javascript:toReturnOrderDetail(\''+order.returnOrderFormList[detailIndex].id+'\')" class=" order_state_btn">查看退款详情</a>';-->
                        <#--}else if(returnType == ${getConstant('dictionary.RETURN_ORDER_TYPE_10')}){-->
                        <#--<!--退货状态&ndash;&gt;-->
                        <#--htmlStr += '<a href="javascript:toReturnOrderDetail(\''+order.returnOrderFormList[detailIndex].id+'\')" class=" order_state_btn">查看退货详情</a>';-->
                        <#--}-->
                        <#--}-->
                        <#--//售后判断是否显示退款/退货按钮-->
                        ...
                        <#--if(checkResult != null){-->
                        <#--if(checkResult == 0){-->
                        <#--htmlStr += '<a href="javascript:applyRefund(\''+order.id+'\',\''+order.orderDetailVoList[detailIndex].id+'\',0)" class=" order_state_btn">申请售后</a>';-->
                        <#--}else{-->
                        <#--htmlStr += '<a href="javascript:applyRefund(\''+order.id+'\',\''+order.orderDetailVoList[detailIndex].id+'\',1)" class=" order_state_btn">修改售后</a>';-->
                        <#--}-->
                        <#--}-->
                        <#--}-->
                        <#--// 查看退款/退货详情按钮-->
                        ...
                        <#--});-->
                            ...
                            Handlebars.registerHelper('orderState', function (totalOrderId, opt) {
                                ...
                                if (order.orderState == '30') {
                                    if (order.orderTrackRemark) {
                                        htmlStr += '(' + order.orderTrackRemark + ')';
                                    }
                                }
//								htmlStr += '<br><a class="order_state_btn" href="javascript:toOrderDetail(\''+ order.id +'\')">查看订单详情</a>';
                                return new Handlebars.SafeString(htmlStr);
                            });
                            ...
                            Handlebars.registerHelper('orderDone', function (totalOrderId, opt) {
                                ...
                                switch (order.orderState) {
                                    ...
                                    case '${getConstant('dictionary.ORDER_STATES_INPAY')}':
                                        /*htmlStr = '<a class="order_state_btn" style="cursor: pointer;" onclick="orderPayment(\''+ order.id +'\',\''+ order.payMoney +'\')">立即付款</a>';*/
                                        <#if Session.user.memberType?? && (Session.user.memberType=="member_servant" || Session.user.memberType=="member_separate")>

                                        <#else >
                                            if (order.tradeAttr1 != "pay_doing") {
                                                htmlStr += '<a href="javascript:void(0)" onclick="toCancelOrder(\'' + order.id + '\')" class="order_state_btn">取消订单</a>';
                                            }
                                        </#if>
...
//										htmlStr += '<a class="order_state_btn" href="javascript:toConfirmReceive(\''+ order.id +'\')">确认收货</a>';
                                        //htmlStr += '<br><a class="order_state_btn" href="javascript:toShippingMessage(\''+ order.id +'\');">查看物流</a>';
                                        //htmlStr += '<br><a href="javascript:void(0);" onclick="extendReceive(\''+ order.id +'\')" class="order_state_btn">延长收货时间</a></td></tr>';
                                        break;
                                        // 已收货 （待评价）
                                    case '${getConstant('dictionary.ORDER_STATES_RECEIPTED')}':       //40
                                    case '${getConstant('dictionary.ORDER_STATES_IN_THE_DISPUTE')}':  //60

                                        //break;
                                        // 交易完成 （已评价）
                                    case '${getConstant('dictionary.ORDER_STATES_COMPLETE')}':  //50
                                        <#if Session.user.memberType?? && (Session.user.memberType=="member_servant" || Session.user.memberType=="member_separate")>
                                        <#else >
                                            if (!order.hasScored) {
                                                htmlStr = '<a href="javascript:toScore(\'' + order.id + '\');" class="order_state_btn">评价交易</a>';
                                            } else {
                                                if (!order.hasSecondScored) {
                                                    htmlStr = "<a href='javascript:checkSecondScore(\"" + order.id + "\")' class='order_state_btn'>追加评价</a>";
                                                }
                                            }
                                        </#if>
                                    ...
                                    case '${getConstant('dictionary.ORDER_STATUS_IN_CHECK')}':
                                        ...
                                        if (serviceStatus == ${getConstant('dictionary.ORDER.SERVICE_STATUS.FIRST_SEND')}) {
                                            htmlStr = "<a href='javascript:toSendBack(\"" + order.id + "\")' class='order_state_btn'>回寄</a>"
                                        }
                                    ...
                                    case '${getConstant('dictionary.ORDER_STATES_PAYED ')}':
                                        /*htmlStr = '<a class="order_state_btn" style="cursor: pointer;" onclick="remindShipping(\''+ order.id +'\')">提醒发货</a>';*/
//										htmlStr = '<span class="table_bgA" >待验收</span>';
                            ...
                            /*'立即购买'按钮处理*/
                            Handlebars.registerHelper('pay_now', function (totalOrderId, tradeAttr1, opt) {
                                var order_data = return_data(opt);
                                var group_id = totalOrderId;
                                var pay_bt = "";
                                var group_price = 0;
                                ...
                                for (var i = 0; i < count(order_data); i++) {
                                    if (order_data[i].totalOrderId == group_id) {
                                        ...
                                        if ($("#showHeadBranchInfo").val() === "0") {
                                            //总子账号提示信息
                                            if (orderMemberType && orderMemberType === MEMBER_MASTER) {

                                                pay_bt += '<div class="od_store_info_box"><span class="od_storeName od_info">采购商店名：' + order_data[i].odBuyerName + '</span><span class="od_passportNo od_info">采购商身份码：' + order_data[i].odPassportNo + '</span><span class="od_memberType od_info">采购商账号类别：' + odMemberType + '</span></div>';
                                            }
                                        } else if ($("#showHeadBranchInfo").val() === "1") {
                                            //总子账号提示信息
                                            if (orderMemberType && (orderMemberType === MEMBER_HEAD || orderMemberType === MEMBER_MASTER)) {
                                                pay_bt += '<div class="od_store_info_box"><span class="od_storeName od_info">采购商店名：' + order_data[i].odBuyerName + '</span><span class="od_passportNo od_info">采购商身份码：' + order_data[i].odPassportNo + '</span><span class="od_memberType od_info">采购商账号类别：' + odMemberType + '</span></div>';
                                            }
                                        } else if ($("#showHeadBranchInfo").val() === "2") {
                                            //总子账号提示信息
                                            if (orderMemberType && ( orderMemberType === MEMBER_MASTER)) {
                                                pay_bt += '<div class="od_store_info_box"><span class="od_storeName od_info">采购商店名：' + order_data[i].odBuyerName + '</span><span class="od_passportNo od_info">采购商身份码：' + order_data[i].odPassportNo + '</span><span class="od_memberType od_info">采购商账号类别：' + odMemberType + '</span></div>';
                                            }
                                        } else if ($("#showHeadBranchInfo").val() === "3") {
                                            //总子账号提示信息
                                            if (orderMemberType && (orderMemberType === MEMBER_MASTER)) {
                                                pay_bt += '<div class="od_store_info_box"><span class="od_storeName od_info">采购商店名：' + order_data[i].odBuyerName + '</span><span class="od_passportNo od_info">采购商身份码：' + order_data[i].odPassportNo + '</span><span class="od_memberType od_info">采购商账号类别：' + odMemberType + '</span></div>';
                                            }
                                        ...
                                        if (order_data[i].orderState == "00") {
                                            ...
                                            group_price += order_data[i].goodsMoney;
                                            it_time = order_data[i].orderDate.split(" ")[0];

                                            if (tradeAttr1 === "pay_doing") {

                                                pay_bt += "<a href='javascript:void(0)'" + " class='ff_button_unc cx1' style='background:#2ab780'>支付中...刷新页面试一试</a>"

                                            } else if ($("#dt_box").attr("businessed") == "false") {

                                                if ($("#dt_box").attr("n_time") != it_time) {
                                                    //集采子账号分采主账号不能下单
                                                    if ((orderMemberType === MEMBER_SERVANT) || (orderMemberType === MEMBER_SEPARATE)) {
                                                      //  pay_bt += "<a href='javascript:void(0)'" + " class='ff_button_unc  cx1'>加入购物车</a>";
                                                    } else if (order_data[i].orderDetailVoList[0].tradeAttr1) {
                                                    } else {
                                                        pay_bt += "<a href='javascript:void(0)'" + " class='big-button cx1' onclick=addToShopCart(" + order_data[i].totalOrderId + ",$(this))>加入购物车</a>";
                                                    }

                                                } else {
                                                    if (orderMemberType === MEMBER_SERVANT || orderMemberType === MEMBER_BRANCH || orderMemberType === MEMBER_SEPARATE) {
                                                        pay_bt += "<a href='javascript:void(0)' class='ff_button_unc cx1'>等待主账号支付</a>"
                                                    } else {
                                                        if ((orderMemberType === MEMBER_HEAD || orderMemberType === MEMBER_MASTER) && opt.data.index != '0' && $("#showHeadBranchInfo").val() === "1") {

                                                        } else if ((orderMemberType === MEMBER_HEAD || orderMemberType === MEMBER_MASTER) && opt.data.index == '0' && $("#showHeadBranchInfo").val() === "1") {
                                                           // pay_bt += "<a href='javascript:void(0)'" + " class='ff_button_unc cx1'>全部支付</a><span style='float: right;color: red;float: none;padding-left: 0.2rem;line-height: 0.8rem'>当前时间点不允许交易，欲知详情请联系客服</span>"
                                                        } else {
                                                            pay_bt += "<a href='javascript:void(0)'" + " class='ff_button_unc cx1'>立即支付</a><span style='float: left;padding-left: 0.05rem;color: red;padding-top: 0.05rem;line-height: 0.7rem'>当前时间点不允许交易，欲知详情请联系客服</span>"
                                                        }
                                                    }
                                                }

                                            } else {
                                                //如果是集采子账号，不能加入购物车；子账号不能支付；总店待支付页面连子店一起支付
                                                if ($("#dt_box").attr("n_time") != it_time) {
                                                    if ((orderMemberType === MEMBER_SERVANT) || (orderMemberType === MEMBER_SEPARATE)) {
                                                        //                                                           pay_bt+="<a href='javascript:void(0)'"+" class='ff_button_unc cx1'>加入购物车</a>";
                                                    } else if (order_data[i].orderDetailVoList[0].tradeAttr1) {
                                                    ...
                                                    if (orderMemberType === MEMBER_SERVANT || orderMemberType === MEMBER_BRANCH || orderMemberType === MEMBER_SEPARATE) {
                                                        ...
                                                        if ((orderMemberType === MEMBER_HEAD || orderMemberType === MEMBER_MASTER) && opt.data.index != '0' && $("#showHeadBranchInfo").val() === "1") {
                                                           ...
                                                           // pay_bt += "<a href='javascript:void(0)'" + " onclick='orderPayment(" + group_id + " ," + group_price + ")' class='big-button cx1'>全部支付</a>"
                                                        } else {
                                                            pay_bt += "<a href='javascript:void(0)'" + " onclick='orderPayment(" + group_id + " ," + group_price + ")' class='big-button cx1'>立即支付</a>"
                                                        }
                                                    }

                                                }
                                            }
                                        } else if (order_data[i].orderState != "00" && order_data[i].orderState != "30") {
                                            if (orderMemberType === MEMBER_SEPARATE) {

                                            } else {
//                                                pay_bt+="<a href='javascript:void(0)'"+" class='print-button cx1' onclick=orderTotalPrint("+order_data[i].totalOrderId+",$(this))>打印采购单</a>";
                                            }
                                        }
...
//                               console.log(pay_bt)
                                return new Handlebars.SafeString(pay_bt);
                            });
                            ...
                            if (dataLength > 10) {
                                ...
                                if (10 * page >= maxNum) {
                                    $(".orderMore").html("没有更多了").attr("onclick", "").addClass('no_more_order');
                                }
                                else {
                                    $(".orderMore").html("加载更多").attr("onclick", "orderMore(this,event)");
                                }
                                pageNum = page
                            }
                            else {
                                if (10 * pageNum >= maxNum) {
                                    $(".orderMore").html("没有更多了").attr("onclick", "").addClass('no_more_order');
                                    ;
                                }
                                else {
    ...
    function isShowSaleAfterButton(order,orderDetail) {
        ...
        var url = "${ctx}/order/getReturnPayed.ajax";
        callService(
                url,
    ...
    function toOrderDetail(orderId) {
        ...
        $.dm.jumpTo(url, {orderId: orderId}, {target: ""});
    }
    ...
    function orderTotalPrint(totalOrderId, el) {
        jsConfirm('确定要打印吗？', {
            callback: function (result) {
                if (result) {
                    callService("/order/printTotalOrder.ajax", {
                        totalOrderId: totalOrderId
                    }, {
                        callSuccess: function (data) {
                            orderTotalPrintFunction(data);
                        }
                    });
                }
            }
    ...
    function orderMore() {
        ...
        pageNum++;
        rememberBrowsePosition(pageNum);
        searchOrder(pageNum);
    }
    //加载更多结束
    //点击单个订单号子元素收起开始
    ...
    //记录上次浏览位置开始
    function rememberBrowsePosition(page) {
        var page = JSON.stringify(page);
        localStorage.browsePage = page;
    }
    //记录上次浏览位置结束
